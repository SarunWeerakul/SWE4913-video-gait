"""
Visualize alignment between two cameras using left–right ankle separation.

We assume input CSVs come from extract_ankle_separation_signal.py and contain:

  frame_idx, ms, sep_px, sep_norm, ...

We:
  - load time (ms) and sep_norm from each file
...
python src/tools/plot_ankle_separation.py \
  --csv1 tmp/UNB-HTL-1001/BF/W1/method1_yolo/cam1/ankle_separation.csv \
  --csv2 tmp/UNB-HTL-1001/BF/W1/method1_yolo/cam4/ankle_separation.csv \
  --label1 cam1 \
  --label2 cam4 \
  --tmin 15000 \
  --tmax 21000
...
def load_sep_csv(path: str | Path) -> Tuple[np.ndarray, np.ndarray]:
    """
    Load ankle_separation.csv and return:
      times_ms: shape (N,)
      signal:   shape (N,) sep_norm
    """
    path = Path(path)
    times: List[float] = []
    vals: List[float] = []
    ...
    if not times:
        raise ValueError(f"No valid sep_norm data in {path}")
...
def visualize_pair(
    ...
    print(f"[plot_ankle_separation] csv1 = {csv1}")
    print(f"[plot_ankle_separation] csv2 = {csv2}")
    if tmin is not None or tmax is not None:
        print(f"[plot_ankle_separation] zoom window = [{tmin}, {tmax}] ms")

    t1, y1 = load_sep_csv(csv1)
    t2, y2 = load_sep_csv(csv2)
    ...
    # Panel 1: raw signals
    axes[0].plot(t_grid, y1g, label=f"{label1} sep_norm")
    axes[0].plot(t_grid, y2g, label=f"{label2} sep_norm", alpha=0.7)
    axes[0].set_ylabel("sep_norm")
    axes[0].set_title("Raw ankle separation vs time")
    axes[0].legend()
    ...
    axes[1].set_ylabel("sep_norm (aligned)")
    axes[1].set_title("Ankle separation with csv2 shifted to align with csv1")
    axes[1].legend()
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot and visualize alignment between two ankle separation signals."
    )
    parser.add_argument("--csv1", required=True, help="ankle_separation.csv for camera 1")
    parser.add_argument("--csv2", required=True, help="ankle_separation.csv for camera 2")
