"""
Extract lower-leg (shank) angle signals from keypoints_common.json.

For each frame we compute:

  Left shank vector  L = ankle_L - knee_L
  Right shank vector R = ankle_R - knee_R
...
python src/tools/extract_shank_angle_signal.py \
  --input tmp/UNB-HTL-1001/BF/W1/method1_yolo/cam1/keypoints_common.json \
  --output tmp/UNB-HTL-1001/BF/W1/method1_yolo/cam1/shank_angle.csv
"""
...
def extract_shank_angles(
    common_data: Dict[str, Any],
    conf_threshold: float = 0.3,
):
    """
    Return list of dict rows with shank angles per frame.
    """
    rows = []
    ...
    for f in common_data.get("frames", []):
        ...
        if lk and la and rk and ra:
            # Check confidence
            if (
                lk["conf"] >= conf_threshold
                and la["conf"] >= conf_threshold
                and rk["conf"] >= conf_threshold
                and ra["conf"] >= conf_threshold
            ):
                # Left shank vector: knee -> ankle
                Lx = la["x"] - lk["x"]
                Ly = la["y"] - lk["y"]

                # Right shank vector: knee -> ankle
                Rx = ra["x"] - rk["x"]
                Ry = ra["y"] - rk["y"]
...
def convert_file(in_path: str | Path, out_path: str | Path, conf_threshold: float):
    ...
    rows = extract_shank_angles(data, conf_threshold=conf_threshold)
    save_csv(rows, out_path)
    print(f"[extract_shank_angle_signal] saved -> {out_path}")


def main():
    parser = argparse.ArgumentParser(
        description="Extract lower-leg (shank) angle signals from keypoints_common.json"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Path to output shank_angle.csv",
    )
    parser.add_argument(
        "--conf-threshold",
        type=float,
